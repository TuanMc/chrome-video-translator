...
### Step 1b: Load Mob Elaboration Context (if available)

**IF** `aidlc-docs/inception/mob-elaboration/session-summary.md` exists:
- Load `aidlc-docs/inception/mob-elaboration/session-summary.md`
- Load per-agent artifacts in that directory (especially `business-analyst.md`, and any agent files relevant to NFRs)
- Treat approved Mob Elaboration outputs as primary elaborations of intent — do not discard or contradict them without logging a conflict and asking the user
- If Mob Elaboration has not run yet but `.aidlc/aidlc-agents/` has agents, load `inception/mob-elaboration.md` and execute Mob Elaboration before continuing Requirements Analysis
