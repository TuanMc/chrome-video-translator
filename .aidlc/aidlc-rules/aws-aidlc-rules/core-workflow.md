...
## MANDATORY: Mob Elaboration Project Agents
**CRITICAL**: When starting or running an AI-DLC **Mob Elaboration** session (Inception elaboration: Requirements Analysis and related planning/design stages), you MUST apply project specialist agents:

1. Load `inception/mob-elaboration.md` from the resolved rule details directory
2. Discover and load every `*.md` agent file under `.aidlc/aidlc-agents/`
3. Follow the Mob Elaboration session protocol in `inception/mob-elaboration.md` (persona turns, no application source-code changes, per-agent artifacts, synthesis + human approval)
4. Continue standard Inception stage rules after Mob Elaboration synthesis is approved

If the user explicitly says "Mob Elaboration" / "start mob elaboration", begin that ritual immediately after Workspace Detection (and Reverse Engineering when required), then proceed through Requirements Analysis using the agent outputs.
...
- Mob Elaboration (ALWAYS when `.aidlc/aidlc-agents/` exists — uses project agents)
...
## Mob Elaboration (ALWAYS WHEN PROJECT AGENTS EXIST)

**Execute IF**:
- `.aidlc/aidlc-agents/` contains one or more `*.md` agent files, OR
- The user explicitly requests Mob Elaboration / a Mob Elaboration session

**Skip IF**:
- No project agents directory/files exist AND the user did not request Mob Elaboration

**Purpose**: Collaborative Inception ritual — apply project specialist agents to turn intent into validated requirements, NFRs, technical direction, and documentation obligations before Construction.

**Execution**:
1. **MANDATORY**: Log start of Mob Elaboration in audit.md
2. Load all steps from `inception/mob-elaboration.md`
3. Discover and load every agent under `.aidlc/aidlc-agents/*.md`
4. Announce loaded agents to the user (no application source-code modifications during this ritual)
5. Run agent turns in the order defined in `inception/mob-elaboration.md`
6. Write per-agent artifacts and `aidlc-docs/inception/mob-elaboration/session-summary.md`
7. **Wait for Explicit Approval** of the Mob Elaboration synthesis — DO NOT PROCEED until user confirms
8. **MANDATORY**: Log user's response in audit.md with complete raw input
9. Proceed to Requirements Analysis using approved Mob Elaboration artifacts as primary input
...
3. **IF Mob Elaboration artifacts exist**: Load `aidlc-docs/inception/mob-elaboration/session-summary.md` and relevant per-agent artifacts before analyzing requirements
4. Execute requirements analysis:
   - Load reverse engineering artifacts (if brownfield)
   - Load Mob Elaboration artifacts (if present)
...
4. **IF present**: Load Mob Elaboration artifacts (`aidlc-docs/inception/mob-elaboration/`), especially `frontend-architect.md`, `accessibility-specialist.md`, and `performance-engineer.md`
5. Execute at appropriate depth (minimal/standard/comprehensive)
