...
A Chrome extension that generates near-real-time Vietnamese subtitles from tab audio (EN/JA/ZH source). Everything runs locally — no paid cloud AI APIs, audio/transcripts never leave localhost in normal operation. Full spec: `.aidlc/inputs/requirement.md`. Deep architecture/API/dependency reference: `aidlc-docs/inception/reverse-engineering/` — read that before making structural changes, don't re-derive it from scratch.

## Layout

Three independently runnable packages. None imports another; they only talk over localhost WebSocket/HTTP at runtime.
...
- **Provider abstraction**: STT and translation are both behind ABCs (`providers/speech_to_text/base.py`, `providers/translation/base.py`). Add new backends as new provider classes, not by branching inside the WebSocket handler.
