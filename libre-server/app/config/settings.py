...
# (~200ms) and must stay cheap. Measured directly on this CPU: a VAD probe
# on a 2s window vs. a full transcribe() pass on real speech — see
# nllb-server/app/config/settings.py for the measured numbers (identical
# faster-whisper model/config, this constant is shared verbatim).
...
# This is the LibreTranslate-backed sibling of ../nllb-server — see its
# README.md for the measured NLLB-vs-LibreTranslate comparison. LibreTranslate
# itself runs as a separate container (see docker-compose.yml); this app talks
# to it over HTTP rather than loading a translation model in-process.
