...
async def translate_ws(websocket: WebSocket) -> None:
    ...
    # Single shared instance from lifespan (see app/main.py) — just an HTTP
    # client, unlike NLLB's per-connection wrapper around a shared in-process
    # model, so there's no need to reconstruct it per session.
    translator = websocket.app.state.translation_provider
